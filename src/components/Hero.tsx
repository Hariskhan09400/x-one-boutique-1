    ...
    <>
      <style>{`
        ...
        .xgrain{
          ...
          opacity:.027;animation:xGr .13s steps(1) infinite;
      ...
      >
        ...
        <div style={{ position:"absolute",inset:0,zIndex:3,pointerEvents:"none" }}>
          ...
          <div style={{ position:"absolute",width:"58vw",height:"58vh",bottom:"-12%",right:"4%",background:`radial-gradient(ellipse,${hsl(H,S,L,.17)} 0%,transparent 68%)`,filter:"blur(82px)",animation:"xBr 7s ease-in-out infinite",transition:"background 1.3s ease" }} />
          <div style={{ position:"absolute",width:"36vw",height:"42vh",top:"-6%",left:"-4%",background:`radial-gradient(ellipse,${hsl(H,S-8,L+6,.08)} 0%,transparent 70%)`,filter:"blur(72px)",animation:"xBr 9s 2s ease-in-out infinite",transition:"background 1.3s ease" }} />
        </div>
        ...
        <div ref={textRef} style={{ position:"absolute",inset:0,zIndex:5,display:"flex",flexDirection:"column",justifyContent:"flex-end",willChange:"transform" }}>
          ...
          <div style={{ position:"absolute",top:0,left:0,right:0,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"clamp(20px,3.5vw,44px) clamp(20px,4vw,60px)",zIndex:10 }}>
            <div key={`chip-${current}`} className="xchip" style={{ display:"flex",alignItems:"center",gap:"8px" }}>
              <div style={{ width:"5px",height:"5px",borderRadius:"50%",background:accent,boxShadow:`0 0 8px ${accent}`,animation:"xPulse 2s ease-in-out infinite",flexShrink:0 }} />
              <span style={{ fontSize:"10.5px",fontWeight:600,letterSpacing:"0.16em",textTransform:"uppercase",color:hsl(H,S,L+8,.9) }}>{slide.tag}</span>
              <span style={{ fontSize:"10px",letterSpacing:"0.12em",textTransform:"uppercase",color:"rgba(255,255,255,.3)",fontWeight:400 }}>/ {slide.season}</span>
            </div>
            <div className="xmhide xant" style={{ fontSize:"11.5px",letterSpacing:"0.2em",color:"rgba(255,255,255,.38)",textTransform:"uppercase" }}>X ONE</div>
          </div>

          {/* Headline block */}
          <div className="xhtext" style={{ padding:"0 clamp(20px,4vw,60px) clamp(140px,20vh,204px)",maxWidth:"clamp(320px,62vw,820px)" }}>
            ...
            <h1 key={`h-${current}`}>
              <span className="xl1 xhl block" style={{ fontSize:"clamp(72px,12.5vw,178px)" }}>{slide.headline[0]}</span>
              <span className="xl2 xhl block" style={{ fontSize:"clamp(72px,12.5vw,178px)",backgroundImage:`linear-gradient(135deg,${hsl(H,S,L+10)} 0%,${hsl(H+20,S-4,L+2)} 55%,${hsl(H+6,S+4,L-10)} 100%)`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",backgroundClip:"text" }}>
                {slide.headline[1]}
              </span>
            </h1>
            <div key={`rule-${current}`} className="xl2" style={{ width:"44px",height:"2px",margin:"18px 0 16px",borderRadius:"2px",background:`linear-gradient(90deg,${accent},transparent)` }} />
            <p key={`sub-${current}`} className="xl3" style={{ fontSize:"clamp(12.5px,1.25vw,14px)",color:"rgba(255,255,255,.4)",maxWidth:"375px",lineHeight:1.82,fontWeight:400 }}>
              {slide.sub}
            </p>
            ...
            <div key={`trust-${current}`} className="xl6" style={{ display:"flex",alignItems:"center",gap:"16px",marginTop:"18px" }}>
              {["Free Shipping","Easy Returns","COD Available"].map(t => (
                <div key={t} style={{ display:"flex",alignItems:"center",gap:"5px" }}>
                  ...
                  <span style={{ fontSize:"9.5px",color:"rgba(255,255,255,.3)",fontWeight:500,letterSpacing:"0.08em",textTransform:"uppercase" }}>{t}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="xtick" style={{ position:"absolute",right:"clamp(18px,2.5vw,32px)",top:"50%",transform:"translateY(-50%)",zIndex:20,display:"flex",flexDirection:"column",alignItems:"center",gap:"10px" }}>
          {slides.map((_, i) => (
            <button key={i} onClick={() => goTo(i)} aria-label={`Slide ${i + 1}`} className="xmag"
              style={{ width:"2px",height:i===current?"38px":"10px",borderRadius:"2px",background:i===current?accent:"rgba(255,255,255,.2)",transition:"height .4s cubic-bezier(.22,1,.36,1),background .3s",border:"none",cursor:"pointer",padding:0 }} />
          ))}
          ...
          <div style={{ background:"linear-gradient(to top,rgba(3,4,10,.97) 0%,rgba(3,4,10,.7) 40%,transparent 100%)",paddingTop:"60px" }}>
            <div className="xcat xcatwrap flex items-center gap-2 overflow-x-auto" style={{ padding:"0 clamp(16px,4vw,60px) 10px" }}>
              {CATEGORIES.map(cat => {
                ...
                return (
                  <button key={cat.label} onClick={() => handleShop(cat.value)} aria-pressed={on}
                    className="xmag xpill shrink-0"
                    style={{ padding:"7px 15px",borderRadius:"100px",fontSize:"11px",fontWeight:on?700:500,letterSpacing:"0.05em",textTransform:"uppercase",background:on?accent:"rgba(255,255,255,.07)",border:`1px solid ${on?accent:"rgba(255,255,255,.1)"}`,color:on?"#fff":"rgba(255,255,255,.42)",boxShadow:on?`0 4px 24px ${hsl(H,S,L,.48)}`:"none",backdropFilter:"blur(16px)" }}>
                    {cat.label}
                  </button>
                );
              })}
            ...
            <div className="xtbar flex items-center gap-4" style={{ padding:"6px clamp(16px,4vw,60px) 24px" }}>
              ...
              <div style={{ flex:1 }} />
              {paused && (
                <span style={{ fontSize:"9px",fontWeight:600,letterSpacing:"0.12em",textTransform:"uppercase",color:"rgba(255,255,255,.24)",flexShrink:0 }}>Paused</span>
              )}
