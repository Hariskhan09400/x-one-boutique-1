...
import CheckoutPage from "./pages/CheckoutPage"; // Isse check kar lena ye import sahi ho
    ...
    <Router>
      ...
      >
        <Routes>
          ...
          <Route
            ...
            element={
              <div className="flex flex-col min-h-screen">
                {/* --- TERI FANCY HEADER (App.tsx wali) --- */}
                  ...
                  <div className="max-w-7xl mx-auto px-4 py-4">
                    <div className="flex items-center justify-between">
                      <a href="/" onClick={(e) => { e.preventDefault(); window.location.reload(); }} className="flex items-center gap-4 group">
                        <div className="px-4 py-2 bg-gradient-to-r from-blue-500 to-cyan-400 rounded-full font-black text-sm shadow-lg shadow-blue-500/30 group-hover:scale-110 transition-all duration-300">
                          XOB
                        </div>
                        <div className="flex flex-col">
                          <div className="text-2xl sm:text-3xl md:text-4xl font-extrabold tracking-wide bg-gradient-to-r from-blue-400 via-cyan-300 to-blue-500 bg-clip-text text-transparent animate-glowBreath">
                            X ONE BOUTIQUE
                          </div>
                          <div className="text-xs text-blue-300 font-semibold tracking-widest mt-1 opacity-80">
                    ...
                    <div className="flex flex-col md:flex-row gap-3 sm:gap-4 mb-4 sm:mb-6">
                      <div className="relative flex-1">
                        ...
                        <input
                          ...
                          className="w-full pl-10 sm:pl-12 pr-4 py-3 rounded-xl bg-gray-100 dark:bg-gray-800 text-black dark:text-white border border-gray-300 dark:border-gray-600 focus:outline-none transition-all"
                        />
                      </div>
                      <select
                        value={sortBy}
                        onChange={(e) => setSortBy(e.target.value as any)}
                        className="px-4 py-3 rounded-xl bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600"
                      >
                      ...
                      <>
                        {[{ id: "jeans", label: "Jeans" }, { id: "Kurta", label: "Kurta" }].map((s) => (
                          <div key={s.id} id={s.id} className="scroll-mt-20">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                              {filteredAndSortedProducts.filter(p => p.category === s.label).map(p => (
                                <ProductCard key={p.id} product={p} onAddToCart={addToCart} onOpenModal={setSelectedProduct} />
                              ))}
                            </div>
                          </div>
          ...
          {/* CHECKOUT PAGE ROUTE (Sahi jagah pe move kar diya) */}
          <Route 
            path="/checkout" 
        ...
        {/* --- GLOBAL COMPONENTS --- */}
        <button onClick={() => setIsCartOpen(true)} className="fixed right-4 bottom-4 z-40 flex items-center gap-2 px-5 py-3 rounded-full font-bold btn-primary shadow-lg">
          <ShoppingCart size={20} /> Cart {cartItemCount > 0 && <span className="w-6 h-6 bg-white text-blue-600 text-xs rounded-full flex items-center justify-center">{cartItemCount}</span>}
        </button>

        <a href="https://wa.me/917208428589" target="_blank" className="fixed left-4 bottom-4 z-40 flex items-center gap-2 bg-green-600 text-white px-5 py-3 rounded-full font-bold shadow-2xl">
          <MessageCircle size={20} /> WhatsApp
        </a>

        {showToast && <div className="fixed bottom-20 left-1/2 -translate-x-1/2 z-50 bg-green-500 text-white px-6 py-3 rounded-full font-bold animate-popIn">Added!</div>}
