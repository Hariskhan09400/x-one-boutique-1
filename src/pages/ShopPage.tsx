...
import { useCart, useWishlist, useAllProducts } from "../App";  // ← useAllProducts added

/* ─────────────────────────────────────────────────────────────────────────────
   ShopPage.tsx — X One Boutique
   • Shows DB products (admin-added) + local products.ts merged
   ───────────────────────────────────────────────────────────────────────────── */

const CATEGORIES = [
  "All", "T-Shirts", "Shirts", "Jeans", "Kurta",
  "Denim Jackets", "Coats", "Belts", "jubba",
];
...
export default function ShopPage() {
  ...
  const { allProducts: products, loading } = useAllProducts(); // ← DB + local merged
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [activeCategory, search, sort, products]); // ← products in deps
  ...
  return (
    <>
      <style>{`
        ...
        .pcard:hover { transform: translateY(-5px); box-shadow: 0 20px 48px rgba(0,0,0,.18) !important; }
        .pcard img { transition: transform .4s ease; }
        .pcard:hover img { transform: scale(1.06); }
        .wish-btn { transition: transform .2s ease, background .2s ease; }
        .wish-btn:hover { transform: scale(1.15); }
        .cart-btn { transition: all .18s ease; }
        .cart-btn:hover { background: #1d4ed8 !important; transform: translateY(-1px); }
        ...
        @keyframes added { 0%{transform:scale(1)} 50%{transform:scale(1.05);background:#16a34a} 100%{transform:scale(1)} }
        .btn-added { animation: added .4s ease; }
      `}</style>

      <div className="xshop" style={{ minHeight: "100vh", background: bg, paddingBottom: "80px" }}>
        ...
        <div style={{
          ...
          <div style={{ maxWidth:"1280px", margin:"0 auto", position:"relative" }}>
            <h1 className="xbb" style={{ fontSize:"clamp(36px,7vw,72px)", color:"#fff", letterSpacing:"0.02em", margin:"0 0 6px" }}>
              OUR <span style={{ color:"#93c5fd" }}>COLLECTION</span>
            </h1>
            <p style={{ fontSize:"14px", color:"rgba(255,255,255,.65)", margin:0 }}>
              {loading ? "Loading..." : `${filtered.length} product${filtered.length !== 1 ? "s" : ""} ${activeCategory !== "All" ? `in ${activeCategory}` : "available"}`}
            </p>
          </div>
        </div>
        ...
        <div style={{ maxWidth:"1280px", margin:"0 auto", padding:"0 16px" }}>
          ...
          {/* Category tabs */}
          <div className="catbar" style={{ display:"flex", gap:"8px", overflowX:"auto", padding:"20px 0 4px" }}>
            {CATEGORIES.map((cat) => {
              const on = activeCategory === cat;
              return (
                <button key={cat} onClick={() => handleCategory(cat)} style={{
                  flexShrink:0, padding:"9px 18px", borderRadius:"30px",
                  fontSize:"13px", fontWeight:700, fontFamily:"inherit", cursor:"pointer",
                  border: on ? "1px solid #2563eb" : `1px solid ${pillBord}`,
                  background: on ? "#2563eb" : pillBg,
                  color: on ? "#fff" : textSec,
                  boxShadow: on ? "0 4px 16px rgba(37,99,235,.4)" : "none",
          ...
          {/* Search + Sort */}
          <div style={{ display:"flex", gap:"10px", margin:"16px 0 24px", flexWrap:"wrap" }}>
            <div style={{ flex:1, minWidth:"200px", position:"relative" }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={textSec} strokeWidth="2.5" strokeLinecap="round"
              ...
              <input type="text" placeholder="Search products..." value={search}
                ...
                style={{
                  width:"100%", padding:"11px 16px 11px 40px", borderRadius:"14px",
                  background:inputBg, border:`1.5px solid ${inputBord}`,
                  color:textPri, fontSize:"14px", fontFamily:"inherit", outline:"none",
                }}
            ...
            <select value={sort} onChange={(e) => setSort(e.target.value as any)}
              style={{
                padding:"11px 16px", borderRadius:"14px", background:inputBg,
                border:`1.5px solid ${inputBord}`, color:textPri,
                fontSize:"13px", fontFamily:"inherit", fontWeight:600, outline:"none", cursor:"pointer", minWidth:"160px",
              }}>
          ...
          {/* Loading state */}
          {loading && (
            <div style={{ textAlign:"center", padding:"60px", color:textSec }}>
              <div style={{ width:"36px", height:"36px", border:"3px solid rgba(37,99,235,.15)", borderTopColor:"#2563eb", borderRadius:"50%", margin:"0 auto 12px", animation:"spin .8s linear infinite" }} />
          ...
          {!loading && filtered.length === 0 && (
            <div style={{ textAlign:"center", padding:"80px 24px", color:textSec }}>
              <div style={{ fontSize:"48px", marginBottom:"16px" }}>🔍</div>
              <p style={{ fontWeight:700, fontSize:"16px", color:textPri, marginBottom:"8px" }}>No products found</p>
          ...
          {!loading && filtered.length > 0 && (
            <>
              {/* Results info */}
              <p style={{ fontSize:"13px", color:textSec, fontWeight:600, marginBottom:"16px" }}>
              ...
              <div className="pgrid">
                {filtered.map((product) => {
                  const img = product.images?.[0] || "https://placehold.co/400x500/1e3a8a/white?text=XOB";
                  ...
                  return (
                    <div key={product.id} className="pcard" style={{
                      ...
                      <Link to={`/product/${product.id}`} style={{
                        ...
                        aspectRatio:"3/4", background: isDark ? "#111" : "#f8fafc",
                      }}>
                        ...
                        <div style={{
                          ...
                          background:"rgba(37,99,235,.85)", backdropFilter:"blur(8px)",
                        ...
                        <button onClick={(e) => { e.preventDefault(); toggleWishlist(product.id); }}
                          className="wish-btn" aria-label="Wishlist"
                          style={{
                            position:"absolute", top:"10px", right:"10px",
                            width:"34px", height:"34px", borderRadius:"50%",
                            background: wishlisted ? "rgba(239,68,68,.9)" : "rgba(255,255,255,.85)",
                            backdropFilter:"blur(8px)", border:"none", cursor:"pointer",
                            display:"flex", alignItems:"center", justifyContent:"center",
                          }}>
                          <svg width="15" height="15" viewBox="0 0 24 24"
                            fill={wishlisted?"#fff":"none"} stroke={wishlisted?"#fff":"#0f172a"}
                      ...
                      <div style={{ padding:"14px 14px 16px" }}>
                        ...
                        <button onClick={() => handleAddToCart(product)}
                          ...
                          style={{
                            ...
                            background: isAdded ? "#16a34a" : "#2563eb", color:"#fff",
              ...
              <p style={{ textAlign:"center", marginTop:"40px", fontSize:"13px", color:textSec, fontWeight:600 }}>
